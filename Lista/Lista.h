...
private:
  ...
  int getTamaRec(Nodo<T> *aux, int contador);
public:
  Lista();
  ...
  void vaciar();
  void print();
  void moverPrimero(T dato);
  void moverSegundo(T dato);
  void moverUltimo(T dato);
  int getTamaRec();
};
...
template <class T> void Lista<T>::moverPrimero(T dato) {
    Nodo<T> *aux;
    aux =  inicio;
    ...
    if(esVacia()) {
        ...
        cout<<"No se encontro dato";
    }
}

template <class T> void Lista<T>::moverSegundo(T dato) {
    Nodo<T> *aux;
    aux= inicio;
    ...
    if(esVacia()) {
        ...
        while(aux->getSiguiente()!=nullptr){
            if(aux->getSiguiente()->getDato()==dato){
                ...
                aux2->setSiguiente(inicio->getSiguiente());
                inicio->setSiguiente(aux2);
                return;
            }else {
                aux = aux->getSiguiente();
            }
...
template <class T> void Lista<T>::moverUltimo(T dato){
    Nodo<T> *aux;
    aux= inicio;
    ...
    if(esVacia()) {
        ...
        if (aux2->getSiguiente()== nullptr){
            aux->setSiguiente(aux2->getSiguiente());
            aux->getSiguiente(aux->getSiguiente());
        }
        cout<<"No se encontro dato";
    }
}

template <class T> int Lista<T>::getTamaRec(Nodo<T> *aux,int contador){
    if(aux == nullptr){
        return contador;
    }else{
        contador++;

        getTamaRec(aux->getSiguiente(),contador);
    }
}
template <class T> int Lista<T>::getTamaRec() {
    Nodo<T> *aux = inicio;
    int contador = 0;
    if(esVacia()){
        return 0;
    } else{
        getTamaRec(aux,contador);
    }
